...
public class InnerScreen {
    ...
    public InnerScreen(Stage stage, List<Map<String, String>> data) {
        this.stage = stage;
        this.data = data;
    }
    ...
    public void showDetails(int index) {
        ...
        mainVBox.getChildren().addAll(nameVBox, locationHBox, priceLabel, line1, descriptionVBox, line2, rateVBox, line3, contactButton,vb);
    ...
    private void updateDetailsView(int index) {
        ...
        if (sideImageView != null) {
            sideImageView.setImage(new Image(data1.get("FarmImplementImage")));
        } else {
            System.out.println("sideImageView is null");
        }

        if (nameLabel != null) {
            nameLabel.setText(data1.get("ProductName"));
        } else {
        ...
        if (priceLabel != null) {
            priceLabel.setText("₹" + data1.get("RentalFees") + "/" + data1.get("RentalDuration"));
        } else {
