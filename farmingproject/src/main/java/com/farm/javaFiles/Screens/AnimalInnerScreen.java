...
public class AnimalInnerScreen {
    ...
    public AnimalInnerScreen(Stage stage, List<Map<String, String>> data) {
        this.stage = stage;
        this.data = data;
    }
    ...
    private void updateDetailsView(int index) {
        ...
        if (sideImageView != null) {
            sideImageView.setImage(new Image(data1.get("AnimalHusbandryImage")));
        } else {
            System.out.println("sideImageView is null");
        }

        if (nameLabel != null) {
            nameLabel.setText(data1.get("AnimalName"));
        } else {
        ...
        if (priceLabel != null) {
            priceLabel.setText("₹" + data1.get("price") );
        } else {
